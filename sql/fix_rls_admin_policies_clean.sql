-- Fix RLS policies for admin access to users table
...
-- Also ensure cooks table has proper RLS policies for admin access
-- Drop any existing cook policies first
DROP POLICY IF EXISTS "Cooks can view their own profile" ON public.cooks;
DROP POLICY IF EXISTS "Cooks can update their own profile" ON public.cooks;
DROP POLICY IF EXISTS "Admins can view all cooks" ON public.cooks;

-- Create RLS policies for cooks table
CREATE POLICY "Cooks can view their own profile or admins can view all" ON public.cooks
FOR SELECT
TO authenticated
USING (
  auth.uid() = id OR public.is_admin(auth.uid())
...
CREATE POLICY "Cooks can update their own profile or admins can update all" ON public.cooks
FOR UPDATE
TO authenticated
USING (
  auth.uid() = id OR public.is_admin(auth.uid())
...
CREATE POLICY "Admins can insert cooks" ON public.cooks
FOR INSERT
TO authenticated
WITH CHECK (
  public.is_admin(auth.uid())
...
CREATE POLICY "Admins can delete cooks" ON public.cooks
FOR DELETE
TO authenticated
USING (
  public.is_admin(auth.uid())
...
-- Ensure RLS is enabled on cooks table
